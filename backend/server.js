...
app.get('/classes', authMiddleware, (req, res) => {
  const classes = db.prepare(`
    SELECT c.id, c.title, c.description, c.code, c.ownerId, u.firstName || ' ' || u.lastName AS ownerName,
...
app.delete('/classes/:id', authMiddleware, (req, res) => {
  const { id } = req.params;
  const userId = req.user.id;

  // Check if user is the owner
  const classInfo = db.prepare('SELECT ownerId FROM classes WHERE id = ?').get(id);
  if (!classInfo) return res.status(404).json({ error: 'Clase no encontrada' });
  if (classInfo.ownerId !== userId) {
...
app.post('/classes/:id/comments', authMiddleware, (req, res) => {
  const classId = Number(req.params.id);
  ...
  const member = db.prepare('SELECT id FROM class_members WHERE classId = ? AND userId = ?').get(classId, userId);
  if (!member && classRow.ownerId !== userId) {
    return res.status(403).json({ error: 'No tienes permiso para comentar en esta clase' });
  }

  const result = db.prepare('INSERT INTO comments (content, authorId, classId) VALUES (?, ?, ?)').run(content, userId, classId);
...
app.post('/designs/:id/copy', authMiddleware, (req, res) => {
  const id = Number(req.params.id);
  const original = db.prepare('SELECT * FROM designs WHERE id = ?').get(id);
...
app.get('/classes/available', authMiddleware, (req, res) => {
  const classes = db.prepare(`
    ...
    FROM classes c
    WHERE c.ownerId = ? 
       OR c.id IN (SELECT classId FROM class_members WHERE userId = ?)
  `).all(req.user.id, req.user.id);
  res.json({ classes });
});
