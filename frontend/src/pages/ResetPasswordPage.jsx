/**
 * ResetPasswordPage — paso 2 de recuperación (/reset-password).
 * Ingresa código + nueva contraseña; permite reenviar código (modificarClave.php).
 */
...
export default function ResetPasswordPage() {
  ...
  // Email recuperado del paso anterior o localStorage
  const recoverEmail = location.state?.email || localStorage.getItem('recoverEmail') || '';
  const [token, setToken] = useState('');
  const [password, setPassword] = useState('');
  const [confirmPassword, setConfirmPassword] = useState('');

  // Reenvía el código de verificación al email guardado
  ...
  // Valida contraseñas y confirma el cambio con código + email
