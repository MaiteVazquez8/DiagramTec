/**
 * AccountPage — panel personal del usuario (/account).
 * Muestra perfil, diseños y clases recientes, enlaces al editor y opción de eliminar cuenta.
 */
...
export default function AccountPage() {
  ...
  // Estado de modales de confirmación (logout / delete)
  const [confirmModal, setConfirmModal] = useState(null);
  const [confirmBusy, setConfirmBusy] = useState(false);
  // Previews de diseños y clases (máx. 5 cada uno)
  ...
  // Carga en paralelo diseños y clases del usuario
  ...
  // Recarga previews cuando hay usuario autenticado
  useEffect(() => {
    if (user) loadPreviews();
  }, [user, loadPreviews]);

  // Placeholders para rellenar la fila cuando hay pocos ítems
  const placeholders = Array.from({ length: 5 });

  // Renderiza una tarjeta de diseño (o placeholder vacío)
  ...
  // Renderiza una tarjeta de clase (o placeholder vacío)
  ...
  return (
    <section className="figma-sector" id="account-page">
      <div className="figma-sector-inner account-layout">
        {/* Barra lateral con perfil y acciones de cuenta */}
        ...
        {/* Contenido principal: previews de diseños y clases */}
      ...
      />
      {/* Modal de confirmación para eliminar cuenta (DELETE /auth/me) */}
      <ConfirmModal
