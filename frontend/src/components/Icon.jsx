...
/**
 * Wrapper de Iconify con mapa de nombres semánticos → identificadores Tabler/Fluent.
 * @see https://icon-sets.iconify.design/tabler/
 */

/** Diccionario de alias internos a iconos de Iconify (Tabler / Fluent). */
...
/**
 * Renderiza un icono SVG vía Iconify a partir de un alias semántico.
 * @param {string} name - Clave en ICONIFY_ICONS (ej. "save", "eye")
 * @param {number} [size=18] - Tamaño en px (ancho y alto)
 * @param {string} [className=''] - Clases CSS adicionales
 * @param {string} [color] - Color explícito; por defecto hereda currentColor
 */
export default function Icon({
  name,
  size = 18,
  className = '',
  strokeWidth: _strokeWidth, // ignorado; Iconify no usa strokeWidth de Lucide
  color,
  style,
  ...props
}) {
  ...
  // Alias desconocido: aviso en consola y no renderiza nada
  if (!icon) {
  ...
  return (
    <IconifyIcon
      ...
      /* Solo oculta de lectores de pantalla si no hay aria-label explícito */
